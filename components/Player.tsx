...
const Player = () => {
  ...
  const handlePlayPause = async () => {
    ...
    if (!res.body) return;
    console.log(
      "🚀 ~ handlePlayPause ~ res.body.is_playing:",
      res.body.is_playing
    );
  ...
  const handleSkipSong = async (skiptTo: "previous" | "next") => {
    if (!deviceId) return;
  ...
  const debounceAdjustVolume = useDebouncedCallback(async (volume: number) => {
